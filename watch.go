...
func Watch(ctx context.Context, r *Robot) error {
	...
	for {
		...
		if result.BowlDetected {
			...
			// Transform point clouds to world frame before saving.
			...
			// Save point clouds to disk for debugging/visualization.
			if err := savePointClouds(r, downsampled, result); err != nil {
				r.logger.Warnf("Failed to save point clouds: %v", err)
			}
...
func savePointClouds(r *Robot, cameraCloud pointcloud.PointCloud, result *applepose.DetectionResult) error {
	outputDir := "pointclouds"
	if err := os.MkdirAll(outputDir, 0o755); err != nil {
		return fmt.Errorf("create output dir: %w", err)
	}

	// Save full camera point cloud (downsampled).
	cameraPath := fmt.Sprintf("%s/camera_full.pcd", outputDir)
	if err := savePointCloudToPCD(cameraCloud, cameraPath); err != nil {
		return fmt.Errorf("save camera cloud: %w", err)
	}
	r.logger.Infof("Saved camera point cloud to %s (%d points)", cameraPath, cameraCloud.Size())

	// Save individual apple point clouds.
	for i, apple := range result.Bowl.Apples {
		if apple.Points != nil && apple.Points.Size() > 0 {
			applePath := fmt.Sprintf("%s/apple_%s.pcd", outputDir, apple.ID)
			if err := savePointCloudToPCD(apple.Points, applePath); err != nil {
				r.logger.Warnf("Failed to save apple %s cloud: %v", apple.ID, err)
				continue
			}
			r.logger.Infof("Saved apple %d point cloud to %s (%d points)", i, applePath, apple.Points.Size())
		}
	}
